...
Scans all `.md` files under root (same as other markdown checks; excludes node_modules, dist, coverage, .git, .husky). Use bold/italic only when explicitly required. Ignores emphasis inside fenced code, inline code, and markdown link blocks `[text](url)` (so underscores in URLs or link text are not flagged).
