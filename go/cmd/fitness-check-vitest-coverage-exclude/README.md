...
- Pass: No Vitest config (falls back to `@mayjournal/fitness-shared` vitest config), or `coverage.exclude` is empty/missing, or every exclude entry ends with `.d.ts`, `.types.ts`, `.test.ts`, or `.spec.ts` (e.g. `src/**/*.types.ts`).
