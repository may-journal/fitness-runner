...
Reads `vitest.config.*` or `package.json` `vitest` in the project root; if none exist, uses the shared vitest config — an installed `@mayjournal/fitness-shared` package when present, else the copy embedded in the check binary, materialized on demand.
