...
classDiagram
    ...
    class loadCheckFromPathOrThrow["5 loadCheckFromPathOrThrow()"]
    ...
    resolveCheckSpecs --> loadCheckFromPathOrThrow : 14
    loadCheck --> Check : 15
    loadCheckFromPathOrThrow --> Check : 16
    run --> getChecks : 17
    run --> runOneCheck : 18
...
| 5   | Dynamic import of a consumer module path; validates `{ name, run }`; marks check path-loaded.                     | Config throws on missing/invalid (`loadCheckFromPathOrThrow`); CLI single-check mode uses the soft `loadCheckFromPath`. |
...
| 13  | Name spec → npm package import.                                                                                   | Resolves the `@mayjournal/fitness-checks/checks/*` subpath.                                                             |
