...
relatedConfigurations: ['../../shared/config/prettier.config.cjs']
---

# prettier

Runs [Prettier](https://prettier.io) `--check` to ensure files are formatted. Uses a local Prettier config when present (e.g. `.prettierrc.json`, `prettier.config.cjs`, or `package.json` `"prettier"` field); otherwise uses `@mayjournal/fitness-shared` `prettier.config.cjs`.
...
- Config: Local project config if present; otherwise `@mayjournal/fitness-shared` prettier config (no copy required in the consumer repo).
